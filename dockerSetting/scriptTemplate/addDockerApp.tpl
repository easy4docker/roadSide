...
docker run -d --restart=on-failure -v "<%=@ondemandResultPath%>":/var/_onDemandOutput -v "<%=@dockerCodePath%>/app":/var/_localApp -v "<%=@dockerDataPath%>":/var/_localAppData --name <%=@siteContainer %>  <%=@siteImage %> 
