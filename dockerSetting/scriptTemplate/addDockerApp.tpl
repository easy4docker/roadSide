...
docker run -d --restart=on-failure --env mainIP="<%=@mainIP %>" --env hostIP="<%=@hostIP %>"  -v "<%=@shareFolder %>":/var/_shareFolder -v "<%=@dockerCodePath%>/app":/var/_localApp -v "<%=@dockerDataPath%>":/var/_localAppData --name <%=@siteContainer %>  <%=@siteImage %> 
